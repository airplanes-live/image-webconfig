#!/usr/bin/env bash
# install.sh — build-mode installer for the feeder image.
#
# Invoked from pi-gen stage-05/00-run.sh with --build-mode. ROOTFS_DIR
# points at the staging rootfs. ARCH is set by pi-gen.
# AIRPLANES_WEBCONFIG_BRANCH is set by the image config (a concrete tag
# for stable, branch name for dev). Downloads the matching GitHub release,
# verifies the manifest's commit_sha equals git rev-parse HEAD of the
# cloned source, installs the binary and rootfs payload into ROOTFS_DIR.
# Does no systemctl, no user creation — those happen in the chroot step
...
echo "image-webconfig install: arch=$ARCH_NAME channel=$CHANNEL tag=$TAG target_root=$TARGET_ROOT"
...
# The caller cloned the source at AIRPLANES_WEBCONFIG_BRANCH. The release
# manifest must point at the same commit, otherwise the baked binary does
# not match the baked rootfs payload (helper scripts, systemd units,
# sudoers). Hard-fail rather than silently ship a mismatch.
