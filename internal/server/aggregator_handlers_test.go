...
func TestAggregatorExport_AttachmentAndNoStore(t *testing.T) {
	t.Parallel()
	h := newWriteHarness(t)
	blob := `{"protocol_version":1,"result":"ok","kind":"aggregator-backup","schema_version":1,"aggregators":{"fr24":{"mlat_enabled":false,"fields":{"email":"a@b.c","sharing_key":"ABCDEF12"}}}}`
	h.stdinResult = wexec.Result{Stdout: []byte(blob)}

	r := postJSON(t, h.client, h.ts.URL+"/api/aggregators/export", map[string]any{})
	defer r.Body.Close()
	if r.StatusCode != http.StatusOK {
		t.Fatalf("status = %d, want 200; body=%s", r.StatusCode, mustReadAll(t, r.Body))
	}
	...
	if !strings.Contains(mustReadAll(t, r.Body), "aggregator-backup") {
		t.Fatal("export body not forwarded")
	}
	calls := h.stdinCallsCopy()
	if len(calls) != 1 || calls[0].argv[len(calls[0].argv)-2] != "export" {
		t.Fatalf("argv = %v", calls)
	}
}

func TestAggregatorImport_PipesBody(t *testing.T) {
	t.Parallel()
	h := newWriteHarness(t)
	h.stdinResult = wexec.Result{Stdout: []byte(`{"protocol_version":1,"result":"ok","imported":1}`)}

	blob := map[string]any{
		"kind":           "aggregator-backup",
		"schema_version": 1,
		"aggregators":    map[string]any{"fr24": map[string]any{"fields": map[string]string{"sharing_key": "ABCDEF12"}}},
	}
	r := postJSON(t, h.client, h.ts.URL+"/api/aggregators/import", blob)
	defer r.Body.Close()
	if r.StatusCode != http.StatusOK {
		t.Fatalf("status = %d, want 200; body=%s", r.StatusCode, mustReadAll(t, r.Body))
	}
	calls := h.stdinCallsCopy()
	if len(calls) != 1 || calls[0].argv[len(calls[0].argv)-2] != "import" {
		t.Fatalf("argv = %v", calls)
	}
	var sent map[string]any
	if err := json.Unmarshal(calls[0].stdin, &sent); err != nil {
		t.Fatalf("import stdin not JSON: %s", calls[0].stdin)
	}
	if sent["kind"] != "aggregator-backup" {
		t.Fatalf("import stdin not piped: %v", sent)
	}
}

func TestAggregatorImport_RejectsNonJSONContentType(t *testing.T) {
	t.Parallel()
	h := newWriteHarness(t)
	req, _ := http.NewRequest(http.MethodPost, h.ts.URL+"/api/aggregators/import", strings.NewReader("kind=backup"))
	...
	if len(h.stdinCallsCopy()) != 0 {
		t.Fatal("helper invoked for non-JSON import body")
	}
}
